/*
  ...
  Print to stdout the siblings of SciDB indexes read from stdin.
...
int main(int argc, char *argv[]) {
  ...
  while( getline(cin, str) && str.length() ) {
    ...
    child  = parent.atLevel(ljSci.getLevel());
    cout << child.getSciDBLeftJustifiedFormat() << endl;
    for(int k=1; k<4; ++k){
      child.setId(child.increment(child.getId(),ljSci.getLevel()));
      cout << child.getSciDBLeftJustifiedFormat() << endl;
    }
  }
  return 0;
