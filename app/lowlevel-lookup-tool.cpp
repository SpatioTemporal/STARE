	...
	try {
		...
		if(text) {
			...
			if(idlookup) {
				...
				sprintf(str,"J2000 %zu %20.16f %20.16f",depth,ra,dec);
				cmd = str;
			} else if (latlon) {
				sprintf(str,"LatLonDegrees %zu %20.16f %20.16f",depth,ra,dec);
				cmd = str;
			} else {
				sprintf(str,"CARTESIAN %zu %20.16f %20.16f %20.16f",depth,x,y,z);
				cmd = str;
			}
